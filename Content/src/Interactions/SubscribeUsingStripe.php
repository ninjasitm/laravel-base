...
use Nitm\Content\Events\Subscription\UserSubscribed;
use Nitm\Content\Contracts\Repositories\UserRepository;
use Nitm\Content\Contracts\Interactions\Subscribe as Contract;
...
class SubscribeUsingStripe implements Contract
{
    ...
    {
        $user->subscriptions()->whereIn('stripe_status', ['incomplete', 'past_due'])->each(
        ...
        if (NitmContent::chargesUsersPerTeam() || NitmContent::chargesUsersPerSeat()) {
            $subscription->quantity(NitmContent::seatsCount($user));
        }
