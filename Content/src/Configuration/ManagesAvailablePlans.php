...
use Nitm\Content\Plan;
use Nitm\Content\Models\UserPlan;
...
trait ManagesAvailablePlans
{
    ...
    {
        return collect(static::$plans)->map(
            function ($plan) {
                $plan->type = 'user';
    ...
    {
        return static::teamPlans()->filter(
            function ($plan) {
                return $plan->interval === 'yearly';
            }
        )->count() > 0;
    ...
    {
        return collect(static::$teamPlans)->map(
            function ($plan) {
                $plan->type = 'team';
