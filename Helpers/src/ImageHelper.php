...
class ImageHelper
{
    ...
    /**
     * Create or get a thumbnail for the image.
     *
     * @param \System\Models\File $image  The image w'ere getting a thumbnailfor
     * @param int                 $width  [description]
     * @param int                 $height [description]
     *
     * @return string The thumbnail URL
     */
    public static function createOrGetThumbnail($image, $width, $height)
    {
        if (!$image) {
            return '';
        }
        ...
        return \Cache::rememberForever(md5($image->getPath().$width.'x'.$height), function () use ($image, $width, $height) {
            try {
                ...
                return $image->getThumb($width, $height, ['mode' => 'crop']);
            } catch (\Exception $e) {
                return '';
            }
        });
    ...
    public static function getPlaceHolderAvatar()
    {
        return '/storage/app/media/images/logos/avatar.png';
    }

    public static function getPlaceHolderBackground()
    {
        return '/storage/app/media/images/logos/logo.png';
    }
}
