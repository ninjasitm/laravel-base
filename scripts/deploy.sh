...
ssh ${SSH_OPTS} ${USER}@${BUILD_URL} "mkdir -p ~${USER}/${APP_FOLDER}/scripts" && scp ${SSH_OPTS} scripts/post_deploy.sh ${USER}@${BUILD_URL}:~${USER}/${APP_FOLDER}/scripts/
...
pwd
cd ~/${APP_FOLDER};
bash scripts/post_deploy.sh --env=${ENV}
ENDSSH
